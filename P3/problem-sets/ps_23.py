...
datadir = "/Users/jpowell/Dropbox/Udacity/DataAnalyst/P3/problem-sets/"
...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for row in table.find_all("tr")[1:]:
            ...
            for e in cells:
                values.append(str(e.text).replace(',', ''))
            ...
            if values[1] != 'TOTAL':
                data.append({"courier": info['courier'],
                            ...
                            "year": int(values[0]),
                            "month": int(values[1]),
                            "flights": {"domestic": int(values[2]),
                                        "international": int(values[3])}
                })
